...
<script lang="ts">
  ...
  export default class extends Vue {
    ...
    kick(member: Member) {
      if (window.confirm(`Kick ${member.displayname}?`)) {
        this.$accessor.user.kick(member)
      }
    }

    ban(member: Member) {
      if (window.confirm(`Ban ${member.displayname}?`)) {
        this.$accessor.user.ban(member)
      }
    }

    mute(member: Member) {
      if (window.confirm(`Mute ${member.displayname}?`)) {
        this.$accessor.user.mute(member)
      }
    }

    unmute(member: Member) {
      if (window.confirm(`Unmute ${member.displayname}?`)) {
